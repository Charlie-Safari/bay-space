export const baySpaceAgreementVersion =
  "BaySpace Privacy Notice and User Agreement v2026-05-21";

export const baySpaceAgreementDocumentHref =
  "/BaySpace_Privacy_Notice_Bluebook_Fm22.pdf?v=2026-05-22";

export const baySpaceAgreementHref = "/agreements/bay-space";

export const bayoPlusAgreementDocumentHref =
  "/may28-updated-crypti-user-agreement-dupy.pdf?v=2026-05-28";

export const bayoPlusAgreementHref = "/agreements/crypti";

export const cryptiAgreementHref = bayoPlusAgreementHref;

export const cryptiAgreementVersion =
  "+CRYPTI User Agreement v2026-05-28";
